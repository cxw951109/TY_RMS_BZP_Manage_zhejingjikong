...
class BllMedicament(Repository):
    ...
    def getDrugList(self, customerId, keyWord, pageParam):
        ...
        orm_query = self.findList().filter(EntityMedicament.CustomerId == customerId
            ...
            or_(EntityMedicament.RFID.like(keyWord), EntityMedicament.Name.like(keyWord))).order_by(
    ...
    def drugUse(self, entityDrug=EntityMedicament(), entityClient=EntityClient(), entityUser=EntityUser()):
        ...
        if (BllUserMedicament().isJInZhiUser(entityUser.UserId, entityDrug.MedicamentId)):
            warning_obj = EntityWarning(WarningId=str(Utils.UUID()), CustomerId=entityDrug.CustomerId,
                                        ...
                                        WarningContent=entityUser.RealName + '违规领用了试剂“' + entityDrug.Name + '”（' + entityDrug.BarCode + '）',
    ...
    def getWaitPutInDrugTemplateList(self, customerId, clientId):
        templateList = BllMedicamentTemplate().findList(and_(EntityMedicamentTemplate.CustomerId == customerId,
                                                             EntityMedicamentTemplate.ClientId == clientId,
                                                             EntityMedicamentTemplate.IsWaitExport == 1)).all()
        return templateList
    ...
    def getAllDrugList(self, _searchWord, pageParam, clientId=''):
        ...
        print('获取试剂列表', queryStr + (' limit ' + str((pageParam.curPage - 1) * pageParam.pageRows) + ',' + str(
    ...
    def select_danger(self, name):
        ...
        dangerData = []
        for i in jsonData:
            dangerData.append({"ename": i["ename"], "cname": i["cname"], "cas": i["cas"], "formula": i["formula"],
                               "wuxing": i["wuxing"], "dulixue": i["dulixue"], "chucunfangfa": i["chucunfangfa"],
                               "yongtu": i["yongtu"], "weight": i["weight"],
                               "xingzhiyuwendingxing": i["xingzhiyuwendingxing"], "dangerpic": i["dangerpic"],
                               "shengtaixue": i["shengtaixue"], "othername": i["othername"], "safestr": i["safestr"]})
        return dangerData
    ...
    def get_boxlist(self, clientId, type, num=0):
        ...
        null_place = ['%s' % i for i in range(1, 181) if i not in query]
